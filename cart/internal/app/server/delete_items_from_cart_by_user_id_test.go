...
func TestHandler_DeleteItemsByUserID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedBody:   "{\"Message\":\"cart emptied successfully\"}\n",
			expectedResp:   nil,
		},
		{
			name:     "err delete all items",
			testData: testData,
			setupMock: func(tc testComponent, mockData model.RequestData) {
				tc.tracer.StartMock.
					Expect(
		...
		{
			name:     "delete all items status OK",
			testData: testData,
			setupMock: func(tc testComponent, mockData model.RequestData) {
				tc.tracer.StartMock.
					Expect(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tc.server.DeleteItemsByUserID(w, req)
