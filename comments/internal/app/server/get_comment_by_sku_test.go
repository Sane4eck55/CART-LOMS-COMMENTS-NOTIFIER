...
func TestHandler_CommentListBySku(t *testing.T) {
	testRequest := &pb.CommentListBySKURequest{
		// nolint:gosec
		Sku:    rand.Int63(),
		Cursor: &pb.Cursor{},
	}

	testComments := []*pb.CommentBySku{
		{
			// nolint:gosec
			Id:        rand.Int63(),
			UserId:    1,
	...
	expectResp := &pb.CommentListBySKUResponse{
	...
	tests := []struct {
		...
		expectedResp       *pb.CommentListBySKUResponse
		expectedErr        error
	}{
		{
			name:        "success",
			...
			setupMock: func(tc testComponent) {
				tc.mock.CommentListBySkuMock.
					Expect(minimock.AnyContext, testRequest.GetSku(), nil).
					Return(testModelComments, nil, nil)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := tc.server.CommentListBySku(context.Background(), tt.testRequest)
